function [tess_gzz] = tess_gzz_field(tesee,glq_lat_order,glq_lon_order,glq_r_order,lonp,latp,rp)
...
for k = 1:1:glq_lon_order
    ...
    for j = 1:1:glq_lat_order
        ...
        for i=1:1:glq_r_order
            ...
            l5 = (l_sqr.^(2.5));
            deltaz = rc.*cospsi - rp;
            res = res+ wlon*wlat*wr*kappa*(3*deltaz.*deltaz - l_sqr)./l5;
        end
    end
end
scale = d2r*(tesee(3) - tesee(2))*d2r*(tesee(5) - tesee(4))*(tesee(7) - tesee(6))/8;
tess_gzz = res*SI2EOTVOS*G*tesee(1)*scale;
end
