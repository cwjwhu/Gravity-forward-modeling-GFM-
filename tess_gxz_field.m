function [tess_gxz] = tess_gxz_field(tesee,glq_lat_order,glq_lon_order,glq_r_order,lonp,latp,rp)
...
for k = 1:1:glq_lon_order
    ...
    for j = 1:1:glq_lat_order
        ...
        for i=1:1:glq_r_order
            ...
            deltax = rc.*kphi;
            deltaz = rc.*cospsi - rp ;
            res = res+ wlon*wlat*wr*kappa*(3*deltax.*deltaz)./(l_sqr.^(2.5)) ;
        end
    end
end
scale = d2r*(tesee(3) - tesee(2))*d2r*(tesee(5) - tesee(4))*(tesee(7) - tesee(6))/8;
tess_gxz = res*SI2EOTVOS*G*tesee(1)*scale;        
end
